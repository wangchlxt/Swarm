...
class Extension extends AbstractConnection
{
     ...
     * @param   string  $ipaddr      optional - IP address of user's location
     * @param   string  $svrname     optional - the user running swarm.
     * @param   string  $svrpass     optional - the user's password that is running swarm.
     *
    ...
    public function __construct(
		...
		//$this->instance->charset = "iso8859-1";
		

        // If connection is coming from user we need to check if remote address and swarm user and password is set
        if ($ipaddr && $svrname && $svrpass) {
            $this->instance->set_evar("ipaddr",   $ipaddr);
            $this->instance->set_evar("svrname",  $svrname);
            $this->instance->set_evar("password", $svrpass);

            // If P4D is running Level 6 the Swarm address and port will need to be set and passed.
            ...
            $this->instance->set_evar("port", $serverAddr.":".$serverPort);
        }
     ...
     *                          no ticket issued (i.e. user has no password).
    ...
    protected function doRun($command, $params = array(), $input = null, $tagged = true)
    {
		...
		//var_dump($this->instance->charset);
