...
use P4\Validate\MultibyteString as MbString;
...
class User extends PluralAbstract
{
    ...
    public static function fetchAll($options = array(), ConnectionInterface $connection = null)
    {
        ...
        // get fetch max option and unset it from the options as we handle it manually
    ...
    /**
     * Get the names of groups that this user belongs to including Subgroups.
     *
    ...
    public function getGroups()
    {
        ...
        return Group::fetchAll(
            array(Group::FETCH_BY_USER => $this->getId(), Group::FETCH_INDIRECT => true),
            $this->getConnection()
        );
    }
    ...
    {
        $group = Group::fetchById($group, $this->getConnection())
            ->addUser($this->getId())
            ->save();
    ...
    {
        $fullName = $this->getRawValue('FullName');
        $enc      = MbString::detectEncoding($fullName);
        // If there are any characters we do not recognize in the full name
        // convert to ASCII so we can at least search without issues
        if (!$enc) {
            $fullName = MbString::convertEncoding($fullName, 'ASCII');
        }
        return $fullName;
    }
     ...
     * then users with invalid emails would be inaccessible.
     *
