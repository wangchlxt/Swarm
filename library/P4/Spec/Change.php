...
class Change extends PluralAbstract implements ResolvableInterface
{
     ...
     */
    public static function fetchById($id, ConnectionInterface $connection = null)
    {
     ...
     *
     * @param   array   $params     optional - pass array('-f') to force delete of another
     *                              user/client's changelist or a submitted (empty) change.
     * @return  Change  provides fluent interface.
     */
        ...
        if ($max === false) {
            foreach (array_keys($this->cache) as $cacheKey) {
                if (strpos($cacheKey, $cacheKeyPrefix) === 0) {
                    $cacheKeyParts = explode('-', $cacheKey);
                    $max           = max($max, end($cacheKeyParts));
                }
            }
        }
