...
abstract class AbstractCounter extends Connected\ConnectedAbstract
{
     ...
     * @param   ConnectionInterface     $connection optional - a specific connection to use.
     * @return  Counter                 instance of the requested counter.
    ...
    public static function fetchAll($options = array(), ConnectionInterface $connection = null)
    {
        ...
        if (isset($options[static::FILTER_MAX]) && $options[static::FILTER_MAX] > 0) {
            // The request has asked for more or less data, give it what it has asked for.
            $max = (int) $options[static::FILTER_MAX];
        }
        $after    = $options[static::FETCH_AFTER];
        $isAfter  = false;
        $filter   = $options[static::FETCH_BY_NAME];
        $pattern  = false;
        $countAll = isset($options[static::FETCH_TOTAL_COUNT]) ? $options[static::FETCH_TOTAL_COUNT] : false;
        ...
        if (!$countAll && $max && !$after && (!$filter || $supportsFilter) && $supportsMax) {
        ...
        if ($countAll) {
            $counters->setProperty('totalCount', $handler->getTotalCount());
        }
     ...
     * point they can decide if they want to expose 'force' or not.
     *
