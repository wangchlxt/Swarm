...
class ClientPool extends ConnectedAbstract
{
    ...
    /**
     * Retrieves the prefix used for client ids.
     *
    ...
    *                                               by default this is false and only local and stream depots are mapped
    ...
    public function reset($clearFiles = true, $stream = null, $mapAllDepots = false)
    {
        ...
        // Attempt to clean workspace first, this might fail due to workspace not existing first time though here.
        try {
            if ($clearFiles) {
                $this->clearFiles();
            }
        } catch (\P4\Exception $e) {
            // Just for debug purpose, we output each time we failed to clean workspace up
            Logger::log(
                Logger::DEBUG,
                "P4 (". spl_object_hash($this) .") Failed to clean workspace (" . $client . ") with error "
                . $e->getMessage()
            );
        }
