...
{
    const MAX_SIZE_VALUE   = 1048576; // 1 MB
    const ALL_FILES        = '//...';
    ...
    public static function fetchAll($query, ConnectionInterface $connection = null)
    {
        ...
        // try/catch parent to deal with the exception we get on non-existent depots
        try {
    ...
    public function setAttributes($attributes, $propagate = true, $force = false)
    {
        ...
        // setup shared initial parameters
        $prefixParams = array();
        if ($propagate) {
            $prefixParams[] = '-p';
        }
    ...
    public function getDepotContents(array $options = null, &$cropped = false)
    {
        ...
        $maxSizeOverride = false;
        $maxSize         = $options[static::MAX_SIZE];
        ...
        if (!array_key_exists('content', $this->cache)) {
            ...
            // don't cache truncated contents - or those where the file size was overridden
            if (!$maxSizeOverride && (!$maxSize || !isset($content[$maxSize + 1]))) {
        ...
        $nonUtf8Encodings = isset($options[Utf8Filter::NON_UTF8_ENCODINGS])
            ? $options[Utf8Filter::NON_UTF8_ENCODINGS]
            : Utf8Filter::$fallbackEncodings;
    ...
    /**
     * Convenience function to return all changes associated with this file.
     *
    ...
    public function getFilelog(array $options = null)
    {
        ...
        $files = array();
        // Use a count as part of the index in case we come across duplicate file names as
        // described in the complicated history https://jira.perforce.com:8443/browse/SW-4354
        $count = 0;
        foreach ($result->getData() as $file => $log) {
            ...
            foreach ($log as $key => $value) {
                ...
                if ($integ !== null) {
                    $files[$count][$file][$index]['integrations'][$integ][$base] = $value;
                } else {
                    $files[$count][$file][$index][$base] = $value;
                }
            }
            $count = $count + 1;
        }
    ...
    {
        return Change::fetchById($this->getStatus('headChange'), $this->getConnection());
    }
