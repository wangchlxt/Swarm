...
class Group extends \P4\Spec\Group
{
    ...
    public static function fetchMembers($id, $options = array(), Connection $connection = null)
    {
        ...
        $recurse = function ($id) use (&$recurse, &$seen, $connection) {
            $group     = Group::fetchById($id, $connection);
        ...
        return static::fetchById($id, $connection)->getUsers();
    }
     ...
     * @return  array               flat list of all members
     */
    public static function fetchUsersAndSubgroups(
        $id,
        Connection $connection = null
    ) {
        ...
        try {
            ...
            Logger::log(Logger::DEBUG, "Couldn't fetch group from cached data");
        }
        ...
        $group                = $groups[$id];
        $allMembers['Users']  = isset($group['Users'])     ? (array) $group['Users']     : array();
        $allMembers['Groups'] = isset($group['Subgroups']) ? (array) $group['Subgroups'] : array();
     ...
     * @return array   flat list of all members
     */
    public static function fetchAllGroupsMembers(array $groups)
    {
        // Iterate over each group and fetch all members
        $members = array();
        foreach ((array) $groups as $group) {
            $members = array_merge($members, static::fetchAllMembers(Group::getGroupName($group)));
        }
    ...
    public static function isMember(
        ...
        switch ($userMode) {
            ...
            case static::USER_MODE_OWNER:
                if (in_array($user, $groups[$group]['Owners'])) {
                    return true;
                }
                break;

            case static::USER_MODE_ALL:
                if (in_array($user, $groups[$group]['Users']) || in_array($user, $groups[$group]['Owners'])) {
                    return true;
                }
                break;
        }
        ...
        if ($recursive) {
            ...
            foreach ($groups[$group]['Subgroups'] as $sub) {
                if (!isset($seen[$sub]) && static::isMember($user, $sub, true, $connection, $seen, $userMode)) {
                    return true;
                }
            }
        }
     ...
     *
     * @param   array|null  $params     pass array('connection' => $connection) to specify the connection to use when
     *                                  deleting the spec
     * @return  void
     */
    public function delete(array $params = null)
    {
        ...
        $specConnection = $params && array_key_exists('connection', $params) ? $params['connection'] : null;
        $this->setConnection($specConnection ?: $connection);
        parent::delete();
        $this->setConnection($connection);
    }
     ...
     * @return mixed the stripped group if it is a group or the value unchanged if not
     */
    public static function getGroupName($groupName)
    {
        return preg_replace('/^' . Config::KEY_PREFIX . '/', '', $groupName);
    }
     ...
     */
    public static function filter($groups, Connection $connection = null, $keyPrefix = true)
    {
        $groupIds = array();
        // we don't want group ids which contain wildcards, isValidId
        // should remove these and any other wacky input values
        foreach ($groups as $key => $group) {
            $groupName = static::getGroupName($group);
            if (static::isValidId($groupName)
                && static::exists($groupName, $connection)) {
                // leverage fetchAll to do the heavy lifting
                $groupId = static::fetchById($groupName, $connection)->getId();
                if (!empty($groupId)) {
