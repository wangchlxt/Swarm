...
class User extends \P4\Spec\User
{
    ...
    public static function fetchById($id, Connection $connection = null)
    {
        ...
        try {
            ...
            // There is an error getting the user - invalidate the cache for next time and fetch
            try {
                $connection->getService('cache')->invalidateItem('users');
            } catch (ServiceNotFoundException $e) {
    ...
    protected static function getCachedUsers(Connection $connection = null)
    {
        ...
        try {
            // Get the users first, that way if we fail to cache the application can still run
        ...
        try {
            ...
            if (strpos($e->getMessage(), 'Cannot open file ') !== false) {
                Logger::log(
                    Logger::WARN,
                    'User: Failed to read user data from cache using fetchAll response instead. ' . $e->getMessage()
                );
                return $users;
            }
            // we pick the best exception to re-throw below, when no users could be returned
        }
