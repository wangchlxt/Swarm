...
class Protections
{
    ...
    public function filterPaths($paths, $mode, $pluck = null)
    {
        ...
        foreach ($paths as $key => $path) {
            ...
            //
            // Note: the first call to P4_Map::join() might be slow if the map size
    ...
    public function limitView(array $view)
    {
        ...
        foreach ($view as $entry) {
            ...
            // We need to check if the server is case sensitive to ensure the case of the
            // depot is the same when we get to the p4_map::join function.
            $caseDepot = $this->isCaseSensitive
                ? $entry['depot']
                : $this->stringToLower($entry['depot']);
            $viewMap->insert('"' . $caseDepot . '" "' . $entry['client'] . '"');
        }
