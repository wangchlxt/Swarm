...
{
    protected $baseUrl;
    // Indicatation of whether we are currently within a code block
    ...
    {
        $trimPattern    = '/^[”’"\'(<]*(.+?)[.”’"\'\,!?:;)>]*$/';
        $calloutPattern = '/^\@{1,2}(?P<starred>\*|!?)(?P<value>[\w]+[\\\\\w\.\-]{0,253}[\w])$/i';
        $groupPattern   = '/^\@@/';
    ...
    public function filter($value)
    {
        ...
        $gotoPattern  = '/^(\@{1,2}\*?([\w\/]+(?:[\\\\\w\/\.\-,!()\'%:#]*[\w\/]|[\w\/])?))$/i';
        $requireOne   = '/^(\@{1,2}\!?([\w\/]+(?:[\\\\\w\/\.\-,!()\'%:#]*[\w\/]|[\w\/])?))$/i';
                    ...
                    if ($length >= $callback['min']) {
                        ...
                        $replace =
                            call_user_func_array(
                                $callback['callback'],
                                array($linkify, $trimmed, $escaper, $lastWord, $baseUrl, $word)
                            );
                        if ($replace !== false) {
                            $trimmed     = $replace;
                            $callbackHit = true;
