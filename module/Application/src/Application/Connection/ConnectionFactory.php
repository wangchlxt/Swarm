...
class ConnectionFactory implements FactoryInterface
{
    ...
    /**
     * The constructor takes the parameters to be passed to the p4 connection factory.
     ...
     * recommended.
     *
     * @param   array   $params     parameters to pass to p4 connection factory
     */
    ...
    public function createService(ServiceLocatorInterface $services)
    {
        ...
        $connection = Connection::factory(
            ...
            null,
            isset($p4['ipaddr'])   ? $p4['ipaddr']   : null,
            isset($p4['svrname'])  ? $p4['svrname']  : null,
            isset($p4['svrpass'])  ? $p4['svrpass']  : null
        );

        if (isset($p4['ipaddr']) && isset($p4['user']) && isset($p4['svrname'])) {
            // Build the message for the log file to give the admin or support help debugging
            $message = "P4 (" . spl_object_hash($this) . ") start command: Authenticating ".$p4['ipaddr']
                . " for user " . $p4['user'];
            // Output the details at level 7.
            $services->get('logger')->log(P4Logger::DEBUG, $message);
        }
