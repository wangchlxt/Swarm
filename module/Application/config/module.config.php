...
return array(
    ...
    'service_manager' => array(
        ...
        'factories' => array(
            'logger'    => function ($services) {
                ...
                $logger = new P4\Log\DynamicLogger(
                    array(
                        'priorities' => array(
                            array(
                                'level' => 8,
                ...
                if ($file && is_writable($file)) {
                    ...
                    $logger->addWriter(new Application\Log\Writer\NullWriter);
                }
            ...
            'p4_user' => function ($services) {
                ...
                // Get the config file and validate that we have enabled proxy mode.
                $config = $services->get('config');
                if (isset($config['p4']['proxy_mode']) && $config['p4']['proxy_mode'] === true) {
                    // set additional settings to allow P4API to valid connection.
                    $p4['ipaddr']  = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : null;
                    $p4['svrname'] = isset($p4['user']) ? $p4['user'] : null;
                    $p4['svrpass'] = isset($p4['password']) ? $p4['password'] : null;
                }
    ...
    'translator' => array(
        ...
        'non_utf8_encodings' => array('windows-1252')
    ),
    'view_manager' => array(
