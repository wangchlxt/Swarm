...
class Project extends AbstractKey
{
    ...
    {
        $defaults =  (array)$this->getRawValue('defaults') + array('reviewers' => array());
        return $defaults;
    }
     ...
     * @param   bool        $flip  if true array keys are the user ids (default is false)
     * @return  array|bool  flat list of all members
     */
    public function getUsersAndSubgroups($flip = false)
    {
        ...
        return Group::fetchUsersAndSubgroups($this->id, $connection);
    }
    ...
    public function getBranches($sortField = null, array $mainlines = array())
    {
        ...
        foreach ($branches as $id => $branch) {
            ...
            $branch['defaults'] += array('reviewers' => array());
    ...
    public function getModerators(array $branches = null)
    {
        ...
        foreach ($this->getBranches() as $branch) {
            if (is_null($branches) || in_array($branch['id'], $branches)) {
                ...
                if ($supportsGroups && $branch['moderators-groups']) {
     ...
     * @param array $reviewers  The existing reviewer list to be merged into.
     ...
     */
    public static function mergeDefaultReviewersForChange(Change $change, array $reviewers, Connection $p4)
    {
        // Firstly get the affected branches
        $affected = Project::getAffectedByChange($change, $p4);
        foreach ($affected as $projectId => $branchIds) {
            ...
            foreach ($projectDefaults['reviewers'] as $reviewerId => $requirement) {
                Project::addRequirement($reviewers, $reviewerId, $requirement, $p4);
            }
     ...
     * @param $reviewers    the currently effective list of reviewers
     * @param $reviewer     the id of the reviewer being added
     * @param $requirement  the level of the current reviewer
     * @param $p4           a perforce connection
     */
    private static function addRequirement(&$reviewers, $reviewer, $requirement, $p4)
    {
        if ((Group::isGroupName($reviewer)
                ? Group::exists(Group::getGroupName($reviewer), $p4)
                : User::exists($reviewer, $p4)) &&
            (! isset($reviewers[$reviewer]) ||
                ((isset($requirement['required']) && $requirement['required'] === true) ||
                    !isset($reviewers[$reviewer]['required'])))) {
    ...
    public function save()
    {
        ...
        $isAdd = false;
        try {
            $group = Group::fetchById($this->id, $this->getConnection());
    ...
    public function isFollowing($userId)
    {
        ...
        $follower = $this->getFollowers($members);
        return in_array($userId, $follower);
    }
     ...
     */
    public function isModerator($userId, array $branches = null)
    {
        if (!$userId) {
            return false;
        }
        $moderators = $this->getModerators($branches);
     ...
     */
    public function getMembershipLevels($userId, $asArray = false)
    {
        if (!$userId) {
            return false;
        }
     ...
     */
    public function getMembershipLevelForSort($userId)
    {
        if ($this->isOwner($userId)) {
            return 1;
        } elseif ($this->isMember($userId)) {
            return 2;
        } elseif ($this->isFollowing($userId)) {
            return 3;
        } elseif ($this->isModerator($userId)) {
            return 4;
        }
        return 0;
    }
     ...
     *
     * @param $readmeLocations   Location(s) to check if there is a readme.md file
     * @param $maxSize           Max size to be displayed
     * @param $markdownHelper    Markdown helper provider.
     * @return readme            The data needed to render the readme
     */
    public function getReadme($readmeLocations, $maxSize, $markdownHelper)
    {
        $project  = $this;
        $p4Admin  = $this->getConnection();
        $branches = $project->getBranches('id', $readmeLocations);
        ...
        $readme = false;
        // Incase array_column doesn't exist use get_field_data
        if (function_exists('array_column')) {
            $branchNames = array_column($branches, 'id');
        } else {
            $branchNames = ArrayValues::getFieldData($branches, 'id');
        }
        foreach ($readmeLocations as $line) {
            if (in_array($line, $branchNames)) {
                $branch = array_filter(
                    $branches,
                ...
                foreach ($branch['paths'] as $depotPath) {
                    ...
                    $filter = Filter::create()->add(
                        ...
                        'headAction',
                        'delete',
                        Filter::COMPARE_NOT_CONTAINS,
                        Filter::CONNECTIVE_AND,
                        true
                    );

                    $query = Query::create()->setFilter($filter);
                    $query->setFilespecs($filePath . '*');
        ...
        if ($readme) {
            $purifiedMarkdown = $markdownHelper->get('markupMarkdown');
            $maxReadMeSize    = $maxSize;

            // If file is over $maxReadMeSize it will display upto that size and cut any ending.
            $contents = $readme->getDepotContents(
                array(
                    $readme::UTF8_CONVERT  => true,
                    $readme::UTF8_SANITIZE => true,
                    $readme::MAX_SIZE      => $maxReadMeSize
                )
            );

            $readme = $purifiedMarkdown($contents);
        } else {
            $readme = '';
        }
        return $readme;
     ...
     * @return mixed the stripped project if it is a project or the value unchanged if not
     */
