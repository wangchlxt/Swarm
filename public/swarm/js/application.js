...
swarm.form = {
    ...
    post: function(url, form, callback, errorNode, prepareData) {
        ...
        $.ajax(url, {
            ...
            error: errorNode ? function(jqXhr, status, message){
        ...
        $.getJSON('/queue/status', function(data) {
            ...
            if (data.workers === 0) {
                warning = swarm.te('Hmm... no queue workers? Ask your administrator to check the')
                    + ' <a href="' + swarm.assetUrl('/docs/#Swarm/setup.worker.html') + '">' + swarm.te("worker setup") + '</a>.';
                ...
                ).render({error: data.pingError, url: swarm.assetUrl('/docs/#Swarm/setup.perforce.html#setup.perforce.triggers')});
            }
