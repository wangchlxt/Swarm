    ...
    search.typeahead({
        source: function(query, process) {
            ...
            delayTimeout = setTimeout(function(){
                ...
                // Set the search filter up with groups by default
                var searchFilters = ['projects', 'groups', 'users', 'files-names', 'files-contents'];
                // If we find the li.super-only class and we don't find super in the body class
                // remove groups from the the searchfilters
                if($('li.super-only').length > 0 && !$('body').hasClass('super')) {
                    searchFilters.splice($.inArray('groups', searchFilters),1);
                }
                $.each(searchFilters, function(i, type){
